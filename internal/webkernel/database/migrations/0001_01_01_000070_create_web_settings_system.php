...
/**
 * Webkernel settings management system.
 *
 * Hierarchical settings with categories, per-module configuration,
...
return new class extends Migration
{
    ...
    {
        // ── instance_setting_categories ───────────────────────────────────────────
        // Grouping and organization of settings.
        Schema::connection($this->connection)->create('instance_setting_categories', function (Blueprint $table) {
        ...
        // ── instance_settings ─────────────────────────────────────────────────────
        // Configuration settings for the system and modules.
        Schema::connection($this->connection)->create('instance_settings', function (Blueprint $table) {
            ...
            $table->foreign('category')
                ->references('key')
                ->on('instance_setting_categories')
                ->cascadeOnDelete();
        });

        // ── instance_settings_history ─────────────────────────────────────────────
        // Audit trail of all setting changes.
        Schema::connection($this->connection)->create('instance_settings_history', function (Blueprint $table) {
            $table->char('id', 26)->primary();
    ...
    {
        Schema::connection($this->connection)->dropIfExists('instance_settings_history');
        Schema::connection($this->connection)->dropIfExists('instance_settings');
        Schema::connection($this->connection)->dropIfExists('instance_setting_categories');
    }
};
