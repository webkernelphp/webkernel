...
namespace Webkernel\Base\System\Ops;
...
use Webkernel\Base\System\Ops\Contracts\Provider;
use Webkernel\Base\System\Ops\Contracts\CrudProvider;
use Webkernel\Base\System\Ops\Contracts\DatabaseSchemaProvider;
use Webkernel\Base\System\Ops\Dto\OperationContext;
use Webkernel\Base\System\Ops\Dto\StepResult;
...
final class OperationBuilder
{
    ...
    private function providerFromUri(string $uri): Provider
    {
        ...
        if (str_starts_with($uri, 'http://') || str_starts_with($uri, 'https://')) {
            return \Webkernel\Base\System\Ops\Providers\ApiProvider::get($uri);
        }

        // File URLs or paths → FileProvider
        if (str_starts_with($uri, 'file://')) {
            $path = substr($uri, 7);
            return \Webkernel\Base\System\Ops\Providers\FileProvider::json($path);
        }

        // CSV files
        if (str_ends_with($uri, '.csv')) {
            return \Webkernel\Base\System\Ops\Providers\FileProvider::csv($uri);
        }

        // JSON files
        if (str_ends_with($uri, '.json')) {
            return \Webkernel\Base\System\Ops\Providers\FileProvider::json($uri);
        }

        // JSONL files
        if (str_ends_with($uri, '.jsonl')) {
            return \Webkernel\Base\System\Ops\Providers\FileProvider::jsonl($uri);
        }

        // Database table → DatabaseProvider
        if (str_starts_with($uri, 'db://')) {
            $table = substr($uri, 5);
            return \Webkernel\Base\System\Ops\Providers\DatabaseProvider::query($table);
        }

        // Assume it's a file path if it exists
        if (is_file($uri)) {
            return match (pathinfo($uri, PATHINFO_EXTENSION)) {
                'csv' => \Webkernel\Base\System\Ops\Providers\FileProvider::csv($uri),
                'jsonl' => \Webkernel\Base\System\Ops\Providers\FileProvider::jsonl($uri),
                default => \Webkernel\Base\System\Ops\Providers\FileProvider::json($uri),
            };
        }
    ...
    private function getArchiveSource(): ?string
    {
        ...
        if ($this->provider instanceof \Webkernel\Base\System\Ops\Providers\ApiProvider) {
        ...
        if ($this->provider instanceof \Webkernel\Base\System\Ops\Providers\FileProvider) {
            try {
