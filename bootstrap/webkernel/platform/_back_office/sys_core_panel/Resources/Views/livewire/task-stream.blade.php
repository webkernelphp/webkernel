<div class="space-y-4" wire:poll.{{ $isRunning ? '500ms' : '5s' }}="render">
    <!-- Task Header Info -->
    <div class="grid grid-cols-4 gap-4">
        ...
        <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Duration</p>
        ...
        <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Type</p>
            <p class="mt-2 text-sm text-gray-900 dark:text-gray-100">
                {{ str($task->type)->replace('_', ' ')->title() }}
            </p>
        </div>

        <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Started</p>
            <p class="mt-2 text-sm text-gray-900 dark:text-gray-100">
                {{ $task->started_at?->format('H:i:s') ?? '—' }}
            </p>
        </div>
    </div>
    ...
    <div class="border border-gray-700 rounded-lg overflow-hidden bg-gray-950">
        ...
        <div class="overflow-auto max-h-96 bg-gray-950">
            <pre class="p-4 text-green-400 font-mono text-sm whitespace-pre-wrap break-words leading-relaxed">@if ($task->output){{ $task->output }}@else<span class="text-gray-500">Waiting for output...</span>@endif</pre>
        </div>
    </div>

    <!-- Error Output (if any) -->
    @if ($task->error)
        <div class="border border-red-800 rounded-lg overflow-hidden bg-red-950/30">
            <div class="bg-red-900/50 px-4 py-2 border-b border-red-800 flex items-center gap-2 text-xs text-red-400 font-mono">
                <span class="text-red-400">✕</span>
                <span>Error</span>
            </div>
            <div class="overflow-auto max-h-48 p-4">
                <pre class="text-red-300 font-mono text-sm whitespace-pre-wrap break-words leading-relaxed">{{ $task->error }}</pre>
            </div>
        </div>
    @endif
</div>
