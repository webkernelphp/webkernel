...
use Webkernel\Integration\Models\WebkernelRelease;
 ...
 * - is_up_to_date is never stored — always computed by the caller from the DB data
 *
 ...
 *   - HTTP webhook from custom update server
 *
 * Rate-limit policy: enforced at the registry level.
 * GitHub free tier: 60 req/h unauthenticated, 5000/h authenticated.
 * A minimum interval between checks can be configured via MIN_CHECK_INTERVAL_SECONDS.
 */
final class WebkernelUpdateChecker
{
    ...
    private const MIN_CHECK_INTERVAL_SECONDS = 300; // 5 min — respect GitHub rate limits
     ...
     * Use only for manual admin actions.
     */
     ...
     *
     * Does NOT throw — errors are logged in the DB and returned via the check record.
     */
    public function check(): WebkernelUpdateCheck
    {
        if (!$this->forceCheck && WebkernelUpdateCheck::isRateLimited($this->targetType, $this->targetSlug)) {
            return $this->logSkip('rate_limited', 'GitHub rate limit active — will retry after reset.');
        }
        ...
        try {
            ...
            // Capture rate-limit state after syncing (free call, doesn't consume quota)
            if ($adapter instanceof GitHubAdapter) {
                try {
                    $rl            = $adapter->rateLimit($source);
                    $rateRemaining = $rl['remaining'];
                    $rateResetAt   = \Illuminate\Support\Carbon::instance($rl['reset_at']);
                } catch (\Throwable) {
     ...
     * Returns null if no check has ever succeeded.
     */
    ...
    /**
     * Compute whether $installedVersion is up to date against the local DB.
     * The is_up_to_date decision is never stored — always computed here at runtime.
     */
    public function isUpToDate(string $installedVersion): bool
    {
        ...
        if ($latest === null) {
            return true; // no data — assume ok until a check runs
        }
    ...
    private function syncToDatabase(array $tags, array $releases): int
    {
        ...
        foreach ($tags as $tag) {
            ...
            if ($existing !== null && isset($releasesByTag[$tagName])) {
            ...
            if (isset($releasesByTag[$tagName])) {
                ...
                // Tag-only (no release): treat as stable unless name suggests otherwise
                $record->published_at = $record->created_at ?? now();
            }
        ...
        // Sync releases that have no matching tag (edge case)
        foreach ($releases as $release) {
    ...
    private function checkedTooRecently(): bool
    {
        ...
        return $last->checked_at->diffInSeconds(now()) < self::MIN_CHECK_INTERVAL_SECONDS;
    }
